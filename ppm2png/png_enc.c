 ...
 *  taken from ? 
 *  (TODO: find origin of table and add a note about it)
 */
...
unsigned long writelongcrc(FILE * fp, unsigned long crc, int l)
{
    ...
    crc = crc ^ 0xFFFFFFFFL;
    crc = crc_table[(crc ^ (*(c+3))) & 0xff] ^ (crc >> 8);
    crc = crc_table[(crc ^ (*(c+2))) & 0xff] ^ (crc >> 8);
    crc = crc_table[(crc ^ (*(c+1))) & 0xff] ^ (crc >> 8);
    crc = crc_table[(crc ^ (*(c+0))) & 0xff] ^ (crc >> 8);
    crc = crc ^ 0xFFFFFFFFL;
...
unsigned long beginchunk(FILE * fp, unsigned long crc, char *name, int len)
{
    ...
    l = len;
    writelong(fp, l);
    write(fp, (unsigned char *)name, 4);

    crc = 0xFFFFFFFFL;
    crc = crc_table[(crc ^ name[0]) & 0xff] ^ (crc >> 8);
    crc = crc_table[(crc ^ name[1]) & 0xff] ^ (crc >> 8);
    crc = crc_table[(crc ^ name[2]) & 0xff] ^ (crc >> 8);
    crc = crc_table[(crc ^ name[3]) & 0xff] ^ (crc >> 8);
    crc = crc ^ 0xFFFFFFFFL;
...
write_png_header(FILE *fp, unsigned long w, unsigned long h) {
    ...
    // write header chank
    crc = beginchunk(fp, crc, "IHDR", 0x0d);
    crc = writelongcrc(fp, crc, w); /* width */
    crc = writelongcrc(fp, crc, h); /* height */
    crc = writebytecrc(fp, crc, 8); /* bit depth */
    crc = writebytecrc(fp, crc, 3); /* color type */
    crc = writebytecrc(fp, crc, 0); /* compression */
    crc = writebytecrc(fp, crc, 0); /* filter */
    crc = writebytecrc(fp, crc, 0); /* interlace */
    crc = endchunk(fp, crc);
...
write_png_palet(FILE *fp, unsigned long crc) {
    ...
    // write palet chank
    crc = beginchunk(fp, crc, "PLTE", 256 * 3);
    for (i = 0; i < 256; i++) {
        crc = writebytecrc(fp, crc, (i & 0xc0));
        crc = writebytecrc(fp, crc, (i & 0x38) << 2);
        crc = writebytecrc(fp, crc, (i & 0x07) << 5);
    }
    crc = endchunk(fp, crc);
...
write_png_data(FILE *fp, unsigned long crc, unsigned long adler_crc,
    ...
    // write png data
    crc = beginchunk(fp, crc, "IDAT", 2 + len + 4);
    // compression method
    crc = writewordcrc(fp, crc, ((0x0800 + 30) / 31) * 31 ); 

    fwrite(data, 1, len, fp);
    crc = update_crc(data, len, crc);
    
    // finish zip part (adler crc)
    crc = writelongcrc(fp, crc, adler_crc);
